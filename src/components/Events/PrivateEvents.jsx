...
import { getPrivateEvents, joinEvent, leaveEvent } from "../../services/event.service.js";
...
export default function PrivateEvents() {
  ...
  return (
    <div className="events-container relative px-4 py-8">
      <h1 className="text-4xl font-bold text-center mb-8 text-gray-800">Private Events</h1>
      <div className="absolute top-0 right-0 mt-4 mr-4 z-10">
        <GoBackArrow onClick={() => navigate("/events")} />
      </div>
      <div className="flex flex-col space-y-6 mt-4">
        {events.length === 0 ? (
          <div className="text-center text-gray-600">No private events found.</div>
        ) : (
          events.map((event) => (
